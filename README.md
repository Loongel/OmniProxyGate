...
首次打开会要求初始化管理员账号。默认 Compose 只把 UI 绑定到 `127.0.0.1:18081`，并使用自签证书提供 HTTPS；浏览器首次访问会提示证书不受信任。不要把管理 UI 裸奔暴露到公网。
...
4. 在“SNI 分流规则”中配置；同一目标后端/动作可以一次录入多个 SNI，ALPN 使用固定选项多选：
   ...
   - `QUIC` 列显示该 SNI 是否允许 HTTP/3 终止后进入 HTTP 路由；默认 `HTTP 开`。
...
- `443 quic reuseport` HTTP/3 终止 server；QUIC 先按 SNI 规则准入，再进入带 Host/泛域名匹配的 HTTP route，`tls_passthrough` 后端不会接收 QUIC 终止后的请求
...
- 默认 Compose 只监听 `127.0.0.1:18081`，UI/API 默认使用 HTTPS。
- API 写操作都要求登录。
- 不允许在 UI 中直接输入任意 Nginx 指令。
- 后端地址、域名、路径、证书路径都会做基础校验。
- Web UI 对多 SNI、多证书域名做聚合录入和聚合展示；底层 API 仍保存为单 SNI / 单域名记录，便于生成器保持确定性。
- Web UI 的 SNI 列表包含 `QUIC` 状态列：`HTTP 开` 表示 `allow_quic_http=true`，`HTTP 关` 表示该 SNI 拒绝 HTTP/3。
