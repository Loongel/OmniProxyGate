...
| 内部 HTTP 终止入口 | 容器内 `0.0.0.0:8443` HTTP 终止监听，stream 内部回连 `127.0.0.1:8443` | 不依赖外部容器 |

`0.0.0.0:8443` 只在容器网络内监听，部署样例不会把 8443 发布到宿主机。加入同一 Docker/Swarm 网络的容器可能访问该端口；如需更强隔离，应避免把不可信服务接入 gateway overlay 网络。
...
| `tls_passthrough` + `allow_quic_http=true` | 只允许进入带 Host/泛域名匹配的 HTTP route；不会使用透传后端 |
| `tls_passthrough` + `allow_quic_http=false` | 拒绝 QUIC |

`allow_quic_http` 默认是 `true`。Web UI 的 SNI 列表用 `QUIC` 列展示该状态：`HTTP 开` 表示允许 QUIC 终止后进入 HTTP route，`HTTP 关` 表示该 SNI 的 QUIC 请求会被拒绝。

如果 SNI 允许 QUIC，但没有任何带 Host/泛域名的 HTTP route 匹配该 SNI，生成器会为该 QUIC SNI 返回 `444`，避免落入全局默认站点或透传后端。带 Host 的 fallback route 也可以作为 QUIC 目标；没有 Host 的全局 fallback 不会放开任意 QUIC SNI。
